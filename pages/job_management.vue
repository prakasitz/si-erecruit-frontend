                ...
                <v-data-table v-model:items-per-page="itemsPerPage" :headers="headers" :items="desserts" item-value="name"
                    ...
                    <template v-slot:top>
                        <v-row>
                            ...
                            <v-col cols="5">
                                <v-text-field v-model="search" density="compact" class="pa-4 rounded-pill"
                                    variant="outlined" label="ค้นหา" append-inner-icon="mdi-magnify"></v-text-field>
                            </v-col>
                        </v-row>
                    </template>
                    ...
                    <template v-slot:[`item.action`]="{ item }">
                        <NuxtLink to="/job_management_detail"><v-icon size="small" class="me-2" @click="editItem(item.raw)">
                                mdi-pencil
                            </v-icon>
                        </NuxtLink>
